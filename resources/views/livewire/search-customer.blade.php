<div>
    ...
    <div class="mt-2">
        <input id="customerSearch" type="text" class="p-4 w-full border rounded-md bg-cool-gray-200 text-black"
            wire:model.live.debounce="searchText" placeholder="Relation nummer zoeken..." value="{{ $searchText }}">
    </div>
        ...
        <div class="mt-4 absolute w-3/4 bg-[#E7F3FC] dark:bg-gray-800 m-auto mb-5 p-5 rounded shadow-md just">
            ...
            @foreach ($results as $result)
                <div class="pt-2 text-black dark:text-white" wire:click="selectResult('{{ $result->id }}')">
                    {{ $result->relation_number }}
                </div>
            @endforeach
        </div>
        <input type="hidden" id="CustmerId" name="CustmerId" value="{{ $selectedId }}">
    </div>
</div>
